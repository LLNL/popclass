...
class NoneClassUQ(additiveUQ):
    def __init__(self,bounds, grid_size=int(1e3), kde=gaussian_kde, kde_kwargs={"bandwidth":0.4}, population_model =None):
        self.bounds = bounds
    ...
    def _build_grids(self):
        ...
        self.grid, self.grid_mesh, self.grid_corners = calculate_square_grid_coordinates(self.gridsize, self.bounds)

        # Calculate bin centers
        # should be (grid_size-1)**d elements
        self.grid_centers, self.grid_mesh_centers, self.grid_centers = calculate_square_grid_centers(self.grid)

        # Calculate grid volumes - Assume fixed grid
        # should be (grid_size-1)**d elements
        self.grid_volumnes = np.product(np.array([self.grid[p][1]-self.grid[p][0] for p in self.grid.keys()])) * np.ones(self.grid_centers.shape)
        return
    ...
    def _train_base_model_kde(self,population_model):
        ...
        base_model_kde = self.kde(pop_model_samples,**kde_kwargs)
    ...
    def apply_uq(self, unnormalized_prob,inference_data, population_model, parameters):
        ...
        pop_model_eval_centers = self.base_model_kde(self.grid_centers)
