...
class NoneClassUQ(additiveUQ):
    ...
    def __init__(self,bounds, grid_size=int(1e3), kde=gaussian_kde, kde_kwargs={}, population_model =None, parameters=None,none_class_weight = .01):
        self.bounds = bounds
    ...
    def _build_grids(self):
        ...
        self.grid, self.grid_mesh, self.grid_corners = calculate_square_grid_coordinates(self.grid_size, self.bounds)

        # Calculate bin centers
        # should be (grid_size-1)**d elements
        self.grid_centers, self.grid_mesh_centers, self.grid_centers_raveled = calculate_square_grid_centers(self.grid)

        # Calculate grid volumes - Assume fixed grid
        # should be (grid_size-1)**d elements
        self.grid_volumnes = np.prod(np.array([self.grid[p][1]-self.grid[p][0] for p in self.grid.keys()])) * np.ones(self.grid_centers_raveled.shape)
        return

    def _train_base_model_kde(self,population_model):
        pop_model_samples = np.vstack([ population_model.samples(class_name, self.parameters) for class_name in population_model.classes])
    ...
    def apply_uq(self, unnormalized_prob,inference_data, population_model, parameters):
        ...
        none_class_pdf_normalization = np.sum(none_class_pdf_centers_unnormed * self.grid_volumes)
        ...
        prior_norm = np.sum(priors.values)/(1.-self.none_class_weight)
