...
color_cycler = ['#009988', '#EE3377', '#EE7733', '#33BBEE', '#CC3311', '#0077BB', "BBBBBB"]
marker_cycler = ['*', '^', 'o', 's', 'H', 'v']

def plot_population_model(PopulationModel, parameters=None, plot_samples=False, plot_kdes=True, bounds=None, N_bins=200, N_hist=40, levels=5, legend=False):
	...
	if bounds is None:
		...
		samples_all = np.concatenate(([PopulationModel.samples(class_name=class_name, parameters=parameters) for class_name in classes]))
		for counter, param in enumerate(parameters):
			param_min, param_max = np.min(samples_all[:,counter]), np.max(samples_all[:,counter])
	...
	for counter, class_name in enumerate(classes):
		...
		if plot_samples:
			if(ndim==1): ax.hist(samples[:,0], color=color_cycler[counter%7], bins=np.linspace(bounds.T[:][0], bounds.T[:][1], N_hist+1).T[0], alpha=0.5, density=True, label=f'{class_name} samples')
			else: ax.scatter(samples[:,0], samples[:,1], color=color_cycler[counter%7], marker=marker_cycler[counter%5], edgecolor='black', s=20, label=f'{class_name} samples')

		if plot_kdes:
			if(ndim==1):
				eval_ = PopulationModel.evaluate_density(class_name=class_name, parameters=parameters, points=coords_eval.swapaxes(0,1))
				ax.plot(coords_eval[0], eval_, color=color_cycler[counter%7], label=f'{class_name} density estimate')
			else:
				eval_ = PopulationModel.evaluate_density(class_name=class_name, parameters=parameters, points=coords_eval.swapaxes(0,1))
				ax.contour(X, Y, eval_.reshape(np.shape(X)), colors=color_cycler[counter%7], linewidths=2, levels=levels)
				legend_proxy = ax.plot(bounds[0] - 1000, bounds[1] - 1000, color=color_cycler[counter%7], lw=2, label=f'{class_name} density estimate')
