from popclass.uq import NoneClassUQ, additiveUQ
...
def test_additiveUQ():
    ...
    classes = ["A","B"]
    parameters = ["p1","p2"]
    samples = {cname : norm.rvs(size=200, loc=1,scale=1).reshape((100,2)) for cname in classes}
    class_weights = {"A":.3,"B":.7}
    population_model = PopulationModel(population_samples = samples, class_weights = class_weights, parameters=parameters)
    ...
    new_probs = additive_uq.apply_uq(unnormalized_prob=unnormalized_prob,inference_data=inference_data, population_model=population_model, parameters = parameters) 
...
def test_none_class_kde_build():
    ...
    nsamples = 100000
    samples = {cname : multivariate_normal.rvs(size=nsamples, mean=means[cname],cov=covs[cname]) for cname in classes}
    class_weights = {"A":float(nsamples)/(nclasses * nsamples),"B":float(nsamples)/(nclasses  * nsamples),"C":float(nsamples)/(nclasses  * nsamples)}

    population_model = PopulationModel(population_samples = samples, class_weights = class_weights, parameters=parameters)
    
    none_class = NoneClassUQ(bounds=bounds, grid_size = grid_size,population_model=population_model,parameters = parameters)

    test_point = np.array([0,0,0])
    true_value = np.sum([class_weights[cname] * multivariate_normal.pdf(test_point, mean = means[cname], cov=covs[cname]) for cname in classes])
